...
- Treat `.kiro/steering/*.md`as the authoritative sources for product, technical, and structural decisions.
...
- Resolve paths for `.kiro/steering` and related directories via approved utilities; avoid absolute paths.
- Modify files only within approved workspace areas and keep `.kiro/steering` untouched directly.
...
- Verify decisions against steering (`.kiro/steering/*.md`) and cite files/sections without duplication.
